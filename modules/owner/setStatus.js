    ...
    if (args.status && /^(?:online|idle|dnd|invisible)$/i.test(args.status)) {
      ...
      message.channel.send({
        embed: {
          ...
          description: `${Bastion.user.username}'s status is now set to **${args.status}** :white_check_mark: `
        }
      }).catch(e => {
        Bastion.log.error(e);
      });
      ...
      message.channel.send({
        embed: {
          color: Bastion.colors.GREEN,
          description: `${Bastion.user.username}'s status is now set to the default status **${Bastion.config.status}** :white_check_mark: `
        }
      }).catch(e => {
        Bastion.log.error(e);
      });
