  ...
  if (!claimedUsers.includes(message.author.id)) {
    ...
    if (Bastion.user.id === '267035345537728512') {
      if (message.guild.id === specialIDs.bastionGuild) {
        ...
        rewardMessage = `Your account has been debited with **${rewardAmount}** Eevee Currencies.`;
      }
      else {
        rewardMessage = `Your account has been debited with **${rewardAmount}** Eevee Currencies.\n\n`
          + '💡 **Pro Tip**\nYou can get **2x** more Eevee Currencies when you use the `claim` or `daily` command in [Bastion HQ](https://discord.gg/fzx8fkt): https://discord.gg/fzx8fkt';
      }
    }
    else {
      rewardMessage = `Your account has been debited with **${rewardAmount}** Eevee Currencies.`;
