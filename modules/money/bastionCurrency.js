...
exports.exec = async (Bastion, message, args) => {
  try {
    ...
    let profile = await Bastion.db.get(`SELECT EeveeCurrencies FROM profiles WHERE userID=${args.id}`), bastionCurrencies = 0;

    if (profile && profile.bastionCurrencies) {
      EeveeCurrencies = profile.bastionCurrencies;
    }
